...
`pymef` / `mef_tools` stack in both directions (94 Python tests + standalone
C++ tests). Scope notes:
...
Uses the active environment's Python. From the repo root:

```bash
scripts/dev_build.sh            # builds build/dev and symlinks the extension
PYTHONPATH=python pytest tests  # requires pymef in the env for the oracle tests
```

Or build a wheel: `python -m build --wheel`.
